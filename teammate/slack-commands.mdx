...
| `/lindy usemodel` | Run your next task on a specific model. |
...
<Note>
  `/lindy usemodel` is available in internal workspaces only for now. Next up on the roadmap is `/lindy redo`, which re-runs a task from its original trigger on a different model.
</Note>
