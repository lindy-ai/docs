...
Secrets are what Lindy reaches for **when it runs shell commands and code**. If you've asked it to hit an API directly rather than through a connected tool, this is where the credential for that call comes from.
...
<Steps>
  ...
  <Step title="Add a new secret">
    Click **Add secret**, then fill in **Secret name** and **Secret value**. Both are required, and **Save** stays greyed out until they are.
  </Step>
  <Step title="Save">
...
<Frame>
  <img src="/lindy-brand-assets/integrations/secrets-add-modal.png" alt="The Add secret dialog, with fields for Secret name and Secret value" />
</Frame>
...
The list shows **Name** and **Updated**, so a rotated secret is easy to spot: its Updated time moves, its name doesn't.
