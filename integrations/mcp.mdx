...
<Warning>
  Reconnecting clears workspace-wide sharing. Once the connection is Active again the owner has to set **Availability** back to **Your workspace**, or the team's tools stay missing. And a stale team connection breaks for everyone at once, so check its status before assuming the server is down.
</Warning>
