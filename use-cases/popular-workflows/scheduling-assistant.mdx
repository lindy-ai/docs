...
Scheduling links can feel impersonal and coordinating meeting times manually is time-consuming. Let your Lindy agent handle the back-and-forth for you!
...
1. **"CC'ing Lindy to find time:"** When the sender asks Lindy to find available times
   - Example: "CC'ing Lindy to find time for us to meet on Tuesday"

2. **"User request to book directly":** When a specific date/time is already agreed upon
   - Example: "Lindy, please book time for us at Thursday the 10th, at 4:00pm PST"

This smart branching ensures Lindy either proposes available times (if none given) OR books the meeting immediately (if time already agreed). The condition prevents Lindy from suggesting times when people have already decided when to meet, making the flow more natural.
