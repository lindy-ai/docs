...
A single Lindy agent can have multiple triggers, either managing different workflows or acting as entry points into the same workflow.
...
If you want your agent to process only emails with a specific label, you can configure the Email Received trigger to filter by label. This way, Lindy will only wake up and run the workflow when an email with that label arrives.
...
<Note>
  **Need help with a specific trigger?**
  ...
  Check out our [integration guides](/skills/popular-integrations) for detailed setup instructions for Gmail, Calendar, Google Sheets, and more.
</Note> 