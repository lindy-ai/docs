...
The list of data that drives each loop iteration.

### Max Cycles
The maximum number of loop iterations that will run. This controls resource usage. Set this based on your expected list size plus a safety buffer.

**Example:** If you expect 50 items, set Max Cycles to 100 to allow for growth while preventing runaway loops.

### Max Concurrent
The maximum number of loop cycles that can run simultaneously in parallel. Higher numbers mean faster processing.

- **Auto ** - Lindy optimizes based on the context
- **Manual** - Set a specific limit (useful for rate-limited APIs or resource constraints)

**When to adjust:** Lower this if you're hitting rate limits on external services, or if parallel execution is causing issues.

### Output
The information passed from inside the loop to subsequent steps. This is the **only** data that survives the loop completion.

- **Auto mode** - AI generates a summary of loop results
- **AI prompt** - Specify exactly what data you need
