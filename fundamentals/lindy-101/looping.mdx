...
- **Items to Loop Through**: The list of data that drives each loop iteration
...
- **Max Concurrent**: The maximum number of loop cycles that can run simultaneously in parallel. Choose "Auto" for optimization or set manually for rate-limited APIs. Lower this if hitting rate limits on external services
- **Output**: The information passed from inside the loop to subsequent steps. This is the **only** data that survives the loop completion. Use "Auto mode" for AI summaries or "AI prompt" to specify exactly what data you need
